...
```shell
ni @dtrensuri/server-shared -D # from @antfu/ni, can be installed via `npm i -g @antfu/ni`
pnpm i @dtrensuri/server-shared -D
yarn i @dtrensuri/server-shared -D
npm i @dtrensuri/server-shared -D
```

```typescript
import type { WebSocketEvents } from '@dtrensuri/server-shared'
```
