...
export const useStageSettingStore = defineStore(
    'suri-v-tube-stage-settings',
    () => {
        const stageModelSelectionStore = useStageModelSelectionStore();

        const stageModelRenderer =
            refManualReset<StageModelRenderer>(undefined);
        const stageModelSelectedUrl = refManualReset<string | undefined>(
